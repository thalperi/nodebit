...
class NodebitWorkspace extends EventEmitter {
        ...
        try {
          ...
          setTimeout(async () => {
            try {
              ...
              this.logger.error('DID/ACL system initialization failed', error)
            }
          }, 2000)
  ...
  async _initializeDIDACLSystem() {
    try {
      ...
      try {
        ...
        this.didRegistry = await this.systemOrbitDB.open('did-registry', {
          type: 'documents'
          // Enable sync for proper persistence - addEventListener issues should be resolved
        })
      ...
      try {
        ...
        this.aclRegistry = await this.systemOrbitDB.open('acl-registry', {
          type: 'documents'
          // Enable sync for proper persistence - addEventListener issues should be resolved
        })
  ...
  async getAllDIDs() {
    ...
    try {
      ...
      this.logger.error('Failed to get DIDs', error)
      throw error
    }
  }
