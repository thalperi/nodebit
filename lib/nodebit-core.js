...
const { createOrbitDB, MemoryStorage } = require('@orbitdb/core')

// Dynamic imports for ES modules
let createHelia, FsBlockstore, FsDatastore, createLibp2p, tcp, webSockets, noise, yamux, identify, gossipsub, Identities, KeyStore, OrbitDBAccessController
...
class NodebitWorkspace extends EventEmitter {
  ...
  async _initializeDIDACLSystem() {
    try {
      ...
      // Wait for Helia's libp2p to be fully started
      const helia = localNetwork.helia;
      if (!helia.libp2p) {
        throw new Error('Helia instance does not have a libp2p property')
      }
      let waitCount = 0;
      while (helia.libp2p.status !== 'started') {
        if (waitCount++ > 50) {
          throw new Error('Helia libp2p did not start in time')
        }
      ...
      // Create system OrbitDB instance for DID/ACL management (official API pattern)
      const uniqueId = crypto.randomBytes(16).toString('hex')
      this.systemOrbitDB = await createOrbitDB({
        ipfs: localNetwork.helia,
        directory: path.join(this.config.dataDir, 'system-orbitdb', uniqueId)
      })
